//This program features segments of code that are reworked or inspired and are NOT MY OWN WORK, references are in the report
...
void reset() //this code takes ideas from a post on the processing website
{
...
void draw()
{
  if(menu == 1) //This segment involves code a youtube that is referenced within the report
  {
    background(58,173,52);
    textAlign(CENTER);   
    fill(0,0,0);
    textSize(72);
    text("Quick Hands",400,400);
    fill(0,0,0);
    textSize(18);
  ...
  if(menu == 2)
  { 
    background(58,173,52);     //the following code was done with the help of a previous lab exercise and features from a post on the processing website
